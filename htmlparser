...
class qis:
    ...
    def check_klausuren(self, username, password):
        ...
        #ausstehende Klausuren auslesen
        exam_button = dr.find_element_by_link_text('Info über angemeldete Prüfungen')
        exam_button.click()
        #dr.implicitly_wait(2)
        dr.find_element_by_xpath('//*[@title="angemeldete Prüfungen anzeigen für Master - Aerospace Engineering  (PO-Version 2016) "]').click()
        #Muss auf jeweiligen Studiengang angepasst werde. Hierzu im QIS das HTML Objekt untersuchen
        print("Ausstehende Klausuren werden gelesen ..")
        #dr.implicitly_wait(2)
        time.sleep(1)
        soup = bs(dr.page_source, 'lxml')
        table = soup.find_all('table')[1] 
        table_body = table.find('tbody')
        rows_open = table_body.find_all('tr')
        ausstehende_klausuren = len(rows_open) - 2 #Minus 2 wegen table headers


        # Eingetragene Prüfungsleistungen auslesen
        res_button = dr.find_element_by_link_text('Notenspiegel')
        res_button.click()
        # dr.implicitly_wait(2)
        dr.find_element_by_xpath(
            '//*[@title="Leistungen für Master - Aerospace Engineering  (PO-Version 2016)  anzeigen"]').click()
        # Muss auf jeweiligen Studiengang angepasst werde. Hierzu im QIS das HTML Objekt untersuchen
        print("Eingetragene Prüfungsergebnisse werden gelesen ..")
        # dr.implicitly_wait(2)
        time.sleep(1)
        soup = bs(dr.page_source, 'lxml')
        table = soup.find_all('table')[1]
        table_body = table.find('tbody')
        rows_result = table_body.find_all('tr')
        eingetragene_klausuren = len(rows_result) - 2  # Minus 2 wegen table headers
        dr.close()
        return ausstehende_klausuren, rows_open, eingetragene_klausuren, rows_result
